...
export default function ProductImage({ productId, alt }: { productId: string; alt: string }) {
  const CDN_URL = "https://cdn.jsdelivr.net/gh/dgocodes/images@master";
  const placeholder = `https://placehold.co/400x400/f3f4f6/2563eb?text=SEM+FOTO`;
  
  // Iniciamos com a URL padrão
  const [imgSrc, setImgSrc] = useState(`${CDN_URL}/${productId}.jpg`);
  const [mounted, setMounted] = useState(false);

  // Garante que o componente só tome decisões de erro após montar no cliente
  useEffect(() => {
    setMounted(true);
  }, []);
  ...
  return (
    <img
      ...
      alt={alt}
      onError={handleError}
      // O 'loading="lazy"' ajuda na performance de grids grandes
      loading="lazy"
      // Se não estiver montado, podemos aplicar uma opacidade 0 para evitar o "flicker"
      className={`
