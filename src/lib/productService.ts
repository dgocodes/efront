...
export const productService = {
   ...
   * Busca principal de produtos com suporte a filtros complexos
   */
  getProdutos: async (filters: SearchFilters): Promise<SearchResponse> => {
    const urlParams = new URLSearchParams();
    ...
    const processFilterValue = (val: string | string[]) => {
      const items = Array.isArray(val) ? val : val.split('_');
      items.forEach(f => { if (f?.trim()) filterSet.add(f.trim()); });
    };
    ...
    // Usando a lib api com suporte a cache do Next.js
    return api.get<SearchResponse>(`/produtos?${urlParams.toString()}`, {
      next: { revalidate: 60 } // Exemplo: Revalida a cada 60 segundos
    });
  },
  ...
  getHomeProducts: async (sort = 'mais-vendidos', pageSize = 10) => {
    return productService.getProdutos({ sort, limit: pageSize });
  },

  getProdutosPorTag: async (tag: string): Promise<SearchResponse> => {
    return api.get<SearchResponse>(`/produtos?tag=${tag}`);
  }
};